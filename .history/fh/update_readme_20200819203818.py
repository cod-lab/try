...
def get_repos_list(user,content):
    ...
    response = r.get('https://api.github.com/users/' + user + '/repos', params=payload, timeout=10).json()        # got list of all user's public repos

    # getting filtered list (latest 5 public repos which r not forked)
    j=1
    for i in range(len(response)):
        if response[i]['fork'] == False:
            content[j+38] = ["* [" + response[i]['name'] + "](" + response[i]['html_url'] + ")\n"]
            j+=1
        if j>5: break
