...
def logs_fn(e):
    ...
    if os.path.exists('Logs') and os.stat('Logs').st_size != 0:
        errs = a.literal_eval(open('Logs','r').read())

    err={}    
    err[len(errs)+1] = {'time':e}
    # err['datenow()']="1no such file"

    errs.update(err)
    print(errs)

    open('Logs','w').write(str(errs))
