...
def get_repos_list(new_list):
    ...
    response = r.get('https://api.github.com/users/' + user + '/repos', params=payload, timeout=0.001)
    print('The server is not responding currently!!\nPlease try again later..')
    result = response.json()        # got list of all repos

    if len(result)<5: s.exit("Error: less than 5 repos available")          # terminate prgm right away after printing msg
...
except FileNotFoundError: s.exit('File not found!!')          # terminate prgm right away after printing msg
print('\nread_file end------------------\n')

print('\nget_repos block----------------------\n')
try: get_repos_list(new_list)
